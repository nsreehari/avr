...
class visitor_registration(models.Model):
        ...
        def button_checkin(self):
            view_id = self.env.ref('avr.visitor_registration_formcheckin').id
        ...
        def button_checkout(self):
            view_id = self.env.ref('avr.visitor_registration_formcheckout').id
            form_name = 'Check-Out Form'
            return self.retform(form_name, view_id)

        @api.multi
        def button_cancel(self):
            view_id = self.env.ref('avr.visitor_registration_formcancel').id
